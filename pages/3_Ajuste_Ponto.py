...
st.set_page_config(page_title="Ajuste de Ponto - Ponto Smart", layout="wide", initial_sidebar_state="expanded")
st.markdown("""
<style>
...
with tab_registros:
    ...
    if not sel_reg:
        ...
        with subtab_add:
            ...
            sem_saida_nova = st.session_state.get("nova_aberto", False)
            with st.form("form_nova_marcacao"):
                ...
                with col_saida:
                    hora_saida_nova = st.time_input(
                        "Hora de Saida",
                        value=None if sem_saida_nova else time(17, 0),
                        key="nova_saida",
                        disabled=sem_saida_nova,
                    )
                ...
                with col_local_out:
                    local_out_nova = st.text_input("Local de Saida", key="nova_location_out", placeholder="Ex: Saida no cliente", disabled=sem_saida_nova)

                col_obs, col_obs_out, col_checkbox = st.columns([2, 2, 1])
                with col_obs:
                    obs_nova = st.text_input("Observacao de Entrada", key="nova_obs")
                with col_obs_out:
                    obs_out_nova = st.text_input("Observacao de Saida", key="nova_obs_out", disabled=sem_saida_nova)
                with col_checkbox:
                    st.markdown("&nbsp;")
                    st.checkbox("Sem saida", key="nova_aberto", label_visibility="visible")
                            ...
                            with st.form(f"form_edit_{reg['id']}"):
                                ...
                                with col_s:
                                    ...
                                    hora_saida = st.time_input(
                                        "Hora de Saida",
                                        value=None if sem_saida else (hora_saida_val or time(17, 0)),
                                        key=f"saida_{reg['id']}",
                                        disabled=sem_saida,
                                    )
